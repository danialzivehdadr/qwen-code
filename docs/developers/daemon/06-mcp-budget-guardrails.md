...
- **`ServeMcpBudgetStatusCell.scope`** is a forward-compat **list** shape (`budgets[]`) rather than a single `budget?` field. PR 14 v1 emits one cell with `scope: 'session'` because each ACP session creates its own `Config` / `McpClientManager` via `acpAgent.newSessionConfig()`. The `'pool'` scope is **reserved** for Wave 5 PR 23 (a pool-scoped cell alongside the session-scoped one) — consumers MUST tolerate additional entries with unrecognized `scope` values (drop them rather than failing) so a future bump can land without a schema break.
...
- F2 design doc (v2.2): [`../../design/f2-mcp-transport-pool.md`](../../design/f2-mcp-transport-pool.md) §11 (workspace-scoped budget specifically) plus the v2.2 changelog rows for W21 / W77 / W88 / W121 / W122 / R3 budget-and-fingerprint fold-ins.

...
- `500 {code: 'compress_failed', sessionId, compressionStatus}` — `tryCompressChat` reported a `COMPRESSION_FAILED_*` status or threw an uncaught exception (transport / auth / OOM mid-side-query). `compressionStatus` carries the specific failure flavor when available, or `'UNKNOWN'` when the agent rethrew a non-typed error (no `compressionStatus` field on the JSON-RPC `data`).

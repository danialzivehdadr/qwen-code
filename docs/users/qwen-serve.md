...
- **Session stats & export** ([#4514](https://github.com/QwenLM/qwen-code/issues/4514) T2.5+T2.6) — fetch per-session aggregate metrics (`GET /session/:id/stats` — promptCount, totalTokens, files touched, context window utilization) or render the conversation in markdown / html / json / jsonl (`GET /session/:id/export?format=md|html|json|jsonl`). Both routes reuse the same `collectSessionData` + `normalizeSessionData` pipeline as the TUI `/export` slash command, so daemon `/stats` and `/export` agree on the numbers for any state already flushed to disk. (The TUI's `/stats` slash command surfaces the same field names but reads in-memory telemetry counters, so it can disagree with daemon `/stats` on a live session until the agent's async write chain drains.) Read-only, no mutation gate; SDK helpers `client.sessionStats(id)` / `client.sessionExport(id, format)` (or session-bound `session.stats()` / `session.export(format)`).
  - **Live-session requirement (v1):** both routes need the session to be live in the daemon. Calls with an unknown session id return `404`; a live session that has not yet written any persisted records returns `400` (`code: invalid_params`, "wait for the first user turn to flush"). To work against an archived session that exists on disk but isn't currently loaded, re-attach it via `POST /session/:id/load` first. The next iteration may relax the live-session requirement for pure-disk reads (no Config needed for the no-context-percent subset).
  - **Token-cost amplification — none:** stats and export are pure reads on the already-flushed JSONL plus an in-process formatter. No LLM side-query, no additional tokens billed; safe to poll.

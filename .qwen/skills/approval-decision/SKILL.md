...
Fetch PR metadata for final guardrails:

```bash
gh pr view "$PR_NUMBER" --repo "$REPO" --json title,body,isCrossRepository,author,changedFiles,additions,deletions
```
...
- **Cross-repository refactor PR** → never auto-approve. Escalate to maintainer
  even if product, review, and tmux all look clean. Refactors from fork authors
  can touch structure broadly and need human maintainer ownership.

...
Apply this gate before template, direction, or approach review.

Core modules include `packages/core/src/**`, package auth/provider/model/config/tool/service areas, and cross-package architectural changes. Maintainer-authored PRs are exempt.

- **Large-scope external core changes**: if a non-maintainer PR changes 500+ lines in core paths (additions + deletions combined), request changes and stop. Large core refactors must be maintainer-initiated. Do not continue to code review.
- **Small-scope external core changes**: proceed only when you are 100% confident the change is correct and you can name the downstream consumers. Any doubt means verdict `needs_human`, not approve/reject.
- **Breadth is not size**: a low-risk sweep across many files is not an automatic rejection if the total core change is small. Flag it for maintainer awareness and judge it under the 100%-confidence rule.

When this gate hard-blocks, write a clear request-changes body. In publish-separation mode, write that body to `$RESULTS_DIR/product-comment.md`, set `"review_action": "request_changes"`, output verdict `fail`, and stop.
...
### 4. Problem Existence and Product Direction

Ask the hard questions before reading code:

- Is the reported problem real and concrete? For bug fixes, require a reproduction, failing test, stack trace, linked issue, or before/after evidence. For features, require a concrete user workflow or maintainer-stated need.
...
If the PR does not demonstrate that the problem exists, request changes and stop. The request should ask for the smallest missing evidence, not a broad redesign. In publish-separation mode, write that body to `$RESULTS_DIR/product-comment.md`, set `"review_action": "request_changes"`, output verdict `fail`, and stop.
...
- `fail` — terminal gate failed: large external core refactor, missing required PR template sections, or missing evidence that the problem exists. Direction and approach concerns normally use `needs_human`, not `fail`.

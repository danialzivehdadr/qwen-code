...
Before judging direction, check whether this PR duplicates an already-open one:

```bash
gh pr list --repo "$REPO" --state open --json number,title,url --limit 100
```

Compare titles and scope against the current PR. If another open PR clearly
solves the same problem (same feature/fix, overlapping files), treat it as a
likely duplicate — **do not auto-reject**: note it in the comment, link the other
PR(s), and escalate (`needs_human`) so the maintainer decides which to keep. A
loose thematic overlap is not a duplicate.

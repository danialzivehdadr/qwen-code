...
const EN: Messages = {
  ...
  'scheduledTasks.runMode.isolated': 'New session each run',
  'scheduledTasks.runMode.shared.hint':
    'Every run continues in one session — later runs see earlier ones.',
  'scheduledTasks.runMode.isolated.hint':
    'Each run is dispatched into a fresh session with a clean context — no shared memory across runs.',
...
const ZH: Messages = {
  ...
  'scheduledTasks.runMode': '运行方式',
  'scheduledTasks.runMode.shared': '共享会话',
  'scheduledTasks.runMode.isolated': '每次运行新建会话',
  'scheduledTasks.runMode.shared.hint':
    '所有运行都在同一个会话里进行 —— 后一次能看到前一次的内容。',
  'scheduledTasks.runMode.isolated.hint':
    '每次运行都派发到一个全新会话、上下文全新 —— 运行之间不共享记忆。',

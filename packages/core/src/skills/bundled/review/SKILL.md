...
**Do not check the coverage. Show it.** Before Step 4, `write_file` **every agent's return, verbatim**, each preceded by a `=== AGENT: <label> ===` line, to `.qwen/tmp/qwen-review-{target}-returns.txt`. The `<label>` MUST carry the agent's assignment: a chunk agent's label is `chunk <id>` (the receipt is credited only when it matches), a whole-diff agent's is its dimension (`security`, `cross-file tracer`). Then run:
...
qwen review check-coverage \
  ...
  --expect "<comma-separated labels you launched>" \
  --out .qwen/tmp/qwen-review-{target}-coverage.json
```

- `--plan` is the **actual** plan path from Step 1 — `.qwen/tmp/qwen-review-pr-<n>-fetch.json` for a PR, but `.qwen/tmp/qwen-review-<target>-plan.json` for a local, file, or cross-repo review. Do not hardcode the fetch report; a local review has no such file and the command would abort.
- `--topology` matches the Step 3 fan-out: `territory` for Step 3B (receipts, one per chunk) or `dimension` for Step 3A (no receipts — every agent walks the whole plan, so the check only asks whether each did a walk). Passing `territory` for a 3A run demands receipts that Step 3A forbids, and blocks the review.
- `--expect` lists every agent you launched. An agent that was never started leaves no return, and without the roster the check cannot miss what is not there — a run that silently dropped the Security agent would pass.

It reads the receipts out of what the agents actually said and tells you which chunks nobody covered, which agents returned nothing, and which expected agents never returned. **It exits 3 when the diff was not covered, and you may not proceed to Step 4 on a non-zero exit** — relaunch each named agent once and run it again (see the relaunch recipe below). **If an agent whiffs a second time**, do not loop: drop it from `--expect`, record its dimension in `unreviewedDimensions` (which caps the verdict at COMMENT just as a missing chunk does), and re-run `check-coverage` without it — it will now exit 0 with that gap disclosed rather than block forever. A twice-whiffed agent is a disclosed unreviewed dimension, not an eternal exit-3.

**The relaunch recipe:** when you re-run an agent, **replace its old block** in the returns file with the new return — do not append. `check-coverage` reads every block, so a bare block left in place keeps the agent on the whiff list forever and the review exits 3 on every re-run. One agent, one block, always its latest return. Its report goes into `compose-review`'s `coverage` input in Step 7, where a failed report forbids an Approve. **Copy the returns; do not summarise them.** A summary of an agent that said nothing says something, and the subcommand would believe it.
...
- **The `event`/`body` decision is computed, not reasoned about.** At submit time a model reasons about what it wants to say rather than what it counted — live reviews proved it five times, so the entire machine (the C/S table, the event-capping overrides, the seven-clause body composition, the downgrade carve-outs) is now a tested subcommand. **Do not hand-derive the event or compose the body.** Gather the run's states into a JSON object and call:
  ...
  - `coverage` — the `check-coverage` report from Step 3, passed through **whole**. **Required for any PR review**: omitting it is itself a cap (`compose-review` refuses to Approve without it), because a run that cannot show what it covered has not shown it covered anything. Its `missingChunks` and `whiffedAgents` become caps here, so a run that cannot show what it covered cannot approve. Omitting it is itself a cap.

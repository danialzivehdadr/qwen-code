...
describe('BackgroundAgentResumeService', () => {
  ...
  it('fires SubagentStart hooks when resuming and injects hook context', async () => {
    ...
    expect(resumedInvocation).toMatchObject({
      version: 1,
      ingress: 'internal',
      sessionId,
    });
    expect(resumedInvocation?.promptId).not.toBe('stale-daemon-prompt');
    expect(resumedInvocation?.originatorClientId).toBeUndefined();
    await vi.waitFor(() => {

 ...
 *
 * The exported `parse` and `stringify` functions provide full YAML spec
 * support with security hardening (prototype pollution prevention,
 * timestamp/binary tag filtering). The Bun-specific wrapper functions
 * (`parseYaml`, `parseYamlSync`, `stringifyYaml`, `parseFrontmatter`,
 * `createFrontmatter`) delegate to Bun.YAML when available for zero-cost
 * parsing in native builds.
 */
...
// ---------------------------------------------------------------------------
// Bun-optimized wrapper functions
// ---------------------------------------------------------------------------

/**
 * Parse YAML string to JavaScript object.
 * Optimized for Bun runtime with built-in YAML parser.
 * Under Node.js, delegates to the full yaml parser.
 */
 ...
 * Uses Bun.YAML under Bun, full yaml parser under Node.js.
 */
 ...
 * Optimized for Bun runtime with built-in YAML parser.
 * Under Node.js, delegates to the full yaml stringifier.
 */
export async function stringifyYaml(
  ...
  return stringify(input as Record<string, unknown>, options);
}

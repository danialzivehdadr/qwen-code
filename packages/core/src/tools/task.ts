...
Usage notes:
1. Launch multiple agents concurrently whenever possible, to maximize performance; to do that, use a single message with multiple tool uses. If one agent depends on another agent's output or side effects, run them sequentially across turns (issue one dependent Task call, wait for its result, then issue the next). Never emit a dependent chain in one message. If dependency is uncertain, default to one Task call per turn.

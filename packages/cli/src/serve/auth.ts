...
export function hostAllowlist(
  ...
  const allowedFor = (port: number): Set<string> => {
    ...
    // no-port forms when listening on a default port (uncommon but
    // valid for an operator who points at a privileged port for clean
    // URLs, or when TLS is terminated on port 443).
    if (port === 80 || port === 443) {
